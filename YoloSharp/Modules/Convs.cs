	...
	internal class Convs
	{
		...
		internal class Conv2 : Conv
		{
			...
			public void fuse_convs()
			{
				...
				w[.., .., i0..(i0 + 1), i1..(i1 + 1)] = cv2.weight.clone();
		...
		internal class Focus : torch.nn.Module<torch.Tensor, torch.Tensor>
		{
			...
			public override torch.Tensor forward(torch.Tensor x)
			{
				...
				var slice1 = x[.., .., 0.., 0..].as_strided(new long[] { x.shape[0], x.shape[1], (x.shape[2] + 1) / 2, (x.shape[3] + 1) / 2 },
					new long[] { x.stride(0), x.stride(1), x.stride(2) * 2, x.stride(3) * 2 });
				var slice2 = x[.., .., 1.., 0..].as_strided(new long[] { x.shape[0], x.shape[1], x.shape[2] / 2, (x.shape[3] + 1) / 2 },
					new long[] { x.stride(0), x.stride(1), x.stride(2) * 2, x.stride(3) * 2 });
				var slice3 = x[.., .., 0.., 1..].as_strided(new long[] { x.shape[0], x.shape[1], (x.shape[2] + 1) / 2, x.shape[3] / 2 },
					new long[] { x.stride(0), x.stride(1), x.stride(2) * 2, x.stride(3) * 2 });
				var slice4 = x[.., .., 1.., 1..].as_strided(new long[] { x.shape[0], x.shape[1], x.shape[2] / 2, x.shape[3] / 2 },
					new long[] { x.stride(0), x.stride(1), x.stride(2) * 2, x.stride(3) * 2 });
