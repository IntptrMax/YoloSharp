...
namespace Data
{
    internal class YoloDataset : BaseDataset
    {
        ...
        public YoloDataset(Config config, bool isValDataset = false) : base(config: config, prefix: YoloSharp.Utils.Display.Color.BrightBlue)
        {
        ...
        public override List<Struct.LabelStruct> get_labels()
        {
            using (torch.no_grad())
            {
                ...
                //(int nkpt, int ndim) = (ValueTuple<int, int>)this.data.GetValueOrDefault("kpt_shape", (0, 0));
                ...
                {
                    //for (int i = 0; i < pbar.Count(); i++)
                    //{
                    //    Struct.LabelStruct label = new Struct.LabelStruct();
                    //    label.im_file = im_files[i];
                    //    torch.Tensor img = torchvision.io.read_image(im_files[i], torchvision.io.ImageReadMode.RGB);
                    ...
                    //    if (File.Exists(label_files[i]))
                    //    {
                    //        string[] lines = File.ReadAllLines(label_files[i]);
                    ...
                    //                using (OpenCvSharp.Mat foreMat = new OpenCvSharp.Mat(mask_h, mask_w, OpenCvSharp.MatType.CV_8UC1, new OpenCvSharp.Scalar(cls[row] + 1f)))
                    //                {
