...
namespace Utils
{
	internal class Ops
	{
		...
		internal static Tensor xywhr2xyxyxyxy(Tensor x)
		{
			...
			return stack(new Tensor[] { pt1, pt2, pt3, pt4 }, -2);
		}
			...
			bool rotated = false, bool end2end = false)
		{
			...
			if (prediction.shape.Last() == 6 || end2end) // end-to-end model (BNC, i.e. 1,300,6)
			{
			   ...
			   .Select(pred =>
			   {
				   ...
				   return pred[mask][max_det];
			   })
			   .ToList(), new List<Tensor> { torch.zeros(0) });
			}
		...
		internal static Tensor nms_rotated(Tensor boxes, Tensor scores, float threshold = 0.45f, bool use_triu = true)
		{
			...
			if (use_triu)
			{
				...
				//Tensor condition = (ious >= threshold).sum(0, type: torch.ScalarType.Bool);
				//pick = torch.nonzero(condition <= 0).squeeze(-1);
