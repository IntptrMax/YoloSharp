    ...
    internal class Loss
    {
        ...
        internal class V8DetectionLoss : Module<Tensor[], Dictionary<string, Tensor>, (Tensor loss, Tensor loss_items)>
        {
            ...
            public override (Tensor loss, Tensor loss_items) forward(Tensor[] preds, Dictionary<string, Tensor> batch)
            {
                using (NewDisposeScope())
                {
                    ...
                    Tensor imgsz = tensor(new long[] { feats[0].shape[2], feats[0].shape[3] }, device: device, dtype: dtype) * stride[0]; // image size (h,w)
        ...
        internal class V8SegmentationLoss : V8DetectionLoss
        {
            ...
            private Tensor calculate_segmentation_loss(Tensor fg_mask, Tensor masks, Tensor target_gt_idx, Tensor target_bboxes, torch.Tensor batch_idx, Tensor proto, Tensor pred_masks, Tensor imgsz, bool overlap)
            {
                using (NewDisposeScope())
                {
                    ...
                    Tensor marea = torchvision.ops.box_convert(target_bboxes_normalized, torchvision.ops.BoxFormats.xyxy, torchvision.ops.BoxFormats.cxcywh)[TensorIndex.Ellipsis, torch.TensorIndex.Slice(2)].prod(2);
        ...
        internal class V8OBBLoss : V8DetectionLoss
        {
            ...
            private Tensor preprocess(Tensor targets, int batch_size, Tensor scale_tensor)
            {
                using (NewDisposeScope())
                {
                    ...
                    {
                        Tensor i = targets[torch.TensorIndex.Ellipsis, 0];  // image index
                        (_, _, Tensor counts) = i.unique(return_counts: true);
                        ...
                        for (int j = 0; j < batch_size; j++)
                        {
                            ...
                            if (n > 0)
                            {
                                Tensor bboxes = targets[TensorIndex.Tensor(matches), torch.TensorIndex.Slice(2)];
                                bboxes[torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 4)].mul_(scale_tensor);
                                @out[j, torch.TensorIndex.Slice(0, n)] = torch.cat(new Tensor[] { targets[TensorIndex.Tensor(matches), torch.TensorIndex.Slice(1, 2)], bboxes }, dim: -1);
                            }
                        }
                    }
            ...
            public override (Tensor loss, Tensor loss_items) forward(Tensor[] preds, Dictionary<string, Tensor> batch)
            {
                using (NewDisposeScope())
                {
                    ...
                    Tensor imgsz = torch.tensor(new long[] { feats[0].shape[2], feats[0].shape[3] }, device: device, dtype: dtype) * this.stride[0];  // image size (h,w)
                    ...
                    try
                    {
                        ...
                        Tensor rw = targets[torch.TensorIndex.Ellipsis, 4] * imgsz[0];
                        Tensor rh = targets[torch.TensorIndex.Ellipsis, 5] * imgsz[1];
        ...
        internal class V8PoseLoss : V8DetectionLoss
        {
            ...
            public override (Tensor loss, Tensor loss_items) forward(Tensor[] preds, Dictionary<string, Tensor> batch)
            {
                ...
                torch.Tensor imgsz = torch.tensor(new long[] { feats[0].shape[2], feats[0].shape[3] }, device: this.device, dtype: dtype) * this.stride[0];  // image size (h,w)
            ...
            private torch.Tensor kpts_decode(torch.Tensor anchor_points, torch.Tensor pred_kpts)
            {
                ...
                y[TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 2)] *= 2.0f;
                y[TensorIndex.Ellipsis, 0] += anchor_points[TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 1)] - 0.5f;
                y[TensorIndex.Ellipsis, 1] += anchor_points[TensorIndex.Ellipsis, torch.TensorIndex.Slice(1, 2)] - 0.5f;
                return y;
            }
            ...
            private (torch.Tensor kpts_loss, torch.Tensor kpts_obj_loss) calculate_keypoints_loss(torch.Tensor masks, torch.Tensor target_gt_idx, torch.Tensor keypoints, torch.Tensor batch_idx, torch.Tensor stride_tensor, torch.Tensor target_bboxes, torch.Tensor pred_kpts)
            {
                using (NewDisposeScope())
                {
                    ...
                    for (int i = 0; i < batch_size; i++)
                    {
                        ...
                        batched_keypoints[i, torch.TensorIndex.Slice(0, (int)keypoints_i.shape[0])] = keypoints_i;
                    }
                    ...
                    if (masks.any().ToBoolean())
                    {
                        ...
                        torch.Tensor area = Ops.xyxy2xywh(target_bboxes[masks])[torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(2)].prod(1, keepdim: true);
