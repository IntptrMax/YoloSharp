	...
	internal class Loss
	{
		...
		private class RotatedBboxLoss : BboxLoss
		{
			...
			public override (Tensor loss_iou, Tensor loss_dfl) forward(Tensor pred_dist, Tensor pred_bboxes, Tensor anchor_points, Tensor target_bboxes, Tensor target_scores, Tensor target_scores_sum, Tensor fg_mask, Tensor imgsz, Tensor stride)
			{
				...
				using (NewDisposeScope())
				{
					...
					if (dfl_loss is not null)
					{
						Tensor target_ltrb = Tal.rbox2dist(target_bboxes[TensorIndex.Ellipsis, ..4], anchor_points, target_bboxes[TensorIndex.Ellipsis, 4..5], reg_max: this.reg_max - 1);
						loss_dfl = this.dfl_loss.forward(pred_dist[fg_mask].view(-1, this.reg_max), target_ltrb[fg_mask]) * weight;
						loss_dfl = loss_dfl.sum() / target_scores_sum;
					}
					else
					{
						Tensor target_ltrb = Tal.rbox2dist(target_bboxes[TensorIndex.Ellipsis, ..4], anchor_points, target_bboxes[TensorIndex.Ellipsis, 4..5]);
						target_ltrb = target_ltrb * stride;
		...
		private class BCEDiceLoss : Module<torch.Tensor, torch.Tensor, torch.Tensor>
		{
			...
			public override Tensor forward(Tensor pred, Tensor target)
			{
				...
				if (target.shape[^2] != mask_h || target.shape[^1] != mask_w)
				{
			...
			public override (Tensor, Tensor) forward(Tensor[] preds, Dictionary<string, Tensor> batch)
			{
				using (NewDisposeScope())
				{
					...
					if (fg_mask.sum().ToSingle() > 0)
					{
						...
						loss[1] = calculate_segmentation_loss(fg_mask, batch["masks"].@float(), target_gt_idx, target_bboxes, batch["batch_idx"].view(-1, 1), proto, pred_masks, imgsz, over_laps);
					}
		...
		internal class V8PoseLoss : V8DetectionLoss
		{
			...
			public V8PoseLoss(int nc = 80, int? kpt_num = null, int? kpt_dim = null, int reg_max = 16, int tal_topk = 10, int[]? stride = null, float hyp_box = 7.5f, float hyp_cls = 0.5f, float hyp_dfl = 1.5f, float hyp_pose = 12.0f, float hyp_kobj = 1.0f) : base(nc: nc, reg_max: reg_max, tal_topk: tal_topk, stride: stride, hyp_box: hyp_box, hyp_cls: hyp_cls, hyp_dfl: hyp_dfl)
			//internal V8PoseLoss()
			{
				this.bce_pose = nn.BCEWithLogitsLoss();
				this.kpt_num = kpt_num ?? 17;
				this.kpt_dim = kpt_dim ?? 3;
				bool is_pose = this.kpt_num == 17 && this.kpt_dim == 3;
				long nkpt = (long)this.kpt_num;  // number of keypoints
			...
			public override (Tensor loss, Tensor loss_items) forward(Tensor[] preds, Dictionary<string, Tensor> batch)
			{
				...
				shape.AddRange(new long[] { (long)this.kpt_num, (long)this.kpt_dim });
		...
		internal class v8DetectionLoss : Module<Dictionary<string, object>, Dictionary<string, Tensor>, (Tensor loss, Tensor loss_detach)>
		{
			...
			private Tensor preprocess(torch.Tensor targets, int batch_size, torch.Tensor scale_tensor)
			{
				using (NewDisposeScope())
				{
					...
					else
					{
						...
						@out[batch_idx, within_idx] = targets[TensorIndex.Colon, 1..];
						@out[TensorIndex.Ellipsis, 1..5] = Ops.xywh2xyxy(@out[TensorIndex.Ellipsis, 1..5].mul_(scale_tensor));
					}
					return @out.MoveToOuterDisposeScope();
				}
			}
			...
			protected ((Tensor fg_mask, Tensor target_gt_idx, Tensor target_bboxes, Tensor anchor_points, Tensor stride_tensor) target, Tensor loss, Tensor loss_detach) get_assigned_targets_and_loss(Dictionary<string, object> preds, Dictionary<string, Tensor> batch)
			{
				using (NewDisposeScope())
				{
					...
					Tensor imgsz = torch.tensor(((Tensor[])preds["feats"])[0].shape[2..], device: this.device, dtype: dtype) * this.stride[0];
		...
		internal class v8OBBLoss : v8DetectionLoss
		{
			...
			private Tensor preprocess(Tensor targets, int batch_size, Tensor scale_tensor)
			{
				using (torch.NewDisposeScope())
				{
					...
					else
					{
						...
						torch.Tensor packed_targets = targets[.., 1..].clone();
						packed_targets[.., 1..5] = packed_targets[.., 1..5].mul_(scale_tensor);
			...
			private (Tensor loss, Tensor loss_items) loss(Dictionary<string, object> preds, Dictionary<string, Tensor> batch)
			{
				using (torch.NewDisposeScope())
				{
					...
					ScalarType dtype = pred_scores.dtype;
					torch.Tensor imgsz = torch.tensor(((torch.Tensor[])preds["feats"])[0].shape[2..], device: this.device, dtype: dtype) * this.stride[0];
		...
		internal class v8SegmentationLoss : v8DetectionLoss
		{
			...
			(Tensor loss, Tensor loss_items) loss(Dictionary<string, object> preds, Dictionary<string, Tensor> batch)
			{
				using (torch.NewDisposeScope())
				{
					...
					if (fg_mask.sum().ToSingle() > 0)
					{
						...
						if (masks.shape[^2] != mask_h || masks.shape[^1] != mask_w) // downsample
						{
							// masks = F.interpolate(masks[None], (mask_h, mask_w), mode="nearest")[0]
							proto = torch.nn.functional.interpolate(proto, new long[] { masks.shape[^2], masks.shape[^1] }, mode: InterpolationMode.Bilinear, align_corners: false);
						}
						torch.Tensor imgsz = torch.tensor(((torch.Tensor[])preds["feats"])[0].shape[2..], device: this.device, dtype: pred_masks.dtype) * this.stride[0];
			...
			private torch.Tensor calculate_segmentation_loss(torch.Tensor fg_mask, torch.Tensor masks, torch.Tensor target_gt_idx, torch.Tensor target_bboxes, torch.Tensor batch_idx, torch.Tensor proto, torch.Tensor pred_masks, torch.Tensor imgsz)
			{
				using (torch.NewDisposeScope())
				{
					...
					// Areas of target bboxes
					torch.Tensor marea = Ops.xyxy2xywh(target_bboxes_normalized)[TensorIndex.Ellipsis, 2..].prod(2);
		...
		internal class v8PoseLoss : v8DetectionLoss
		{
			...
			private (Tensor loss, Tensor loss_items) loss(Dictionary<string, object> preds, Dictionary<string, Tensor> batch)
			{
				using (torch.NewDisposeScope())
				{
					...
					torch.Tensor imgsz = torch.tensor(((torch.Tensor[])preds["feats"])[0].shape[2..], device: this.device, dtype: pred_kpts.dtype) * this.stride[0];
			...
			{
				torch.Tensor y = pred_kpts.clone();
				y[TensorIndex.Ellipsis, ..2] *= 2.0;
				y[TensorIndex.Ellipsis, 0] += anchor_points[TensorIndex.Colon, 0..1] - 0.5f;
				y[TensorIndex.Ellipsis, 1] += anchor_points[TensorIndex.Colon, 1..2] - 0.5f;
				return y;
			}
			...
			private (Tensor kpts_loss, Tensor kpts_obj_loss) calculate_keypoints_loss(torch.Tensor masks, torch.Tensor target_gt_idx, torch.Tensor keypoints, torch.Tensor batch_idx, torch.Tensor stride_tensor, torch.Tensor target_bboxes, torch.Tensor pred_kpts)
			{
				using (torch.NewDisposeScope())
				{
					...
					if (masks.any().ToBoolean())
					{
						target_bboxes /= stride_tensor;
						torch.Tensor gt_kpt = selected_keypoints[masks];
						torch.Tensor area = Ops.xyxy2xywh(target_bboxes[masks])[TensorIndex.Colon, 2..].prod(1, keepdim: true);
						torch.Tensor pred_kpt = pred_kpts[masks];
						torch.Tensor kpt_mask = gt_kpt.shape[^1] == 3 ? gt_kpt[TensorIndex.Ellipsis, 2] != 0 : torch.full_like(gt_kpt[TensorIndex.Ellipsis, 0], true);
						kpts_loss = this.keypoint_loss.forward(pred_kpt, gt_kpt, kpt_mask, area);  // pose loss
