	...
	internal class Detector : YoloBaseTaskModel
	{
		...
		internal override List<YoloResult> ImagePredict(Tensor orgImage, float predictThreshold, float iouThreshold)
		{
			using (no_grad())
			{
				...
				Tensor nms_results = Ops.non_max_suppression(inference["boxes"], predictThreshold, iouThreshold, end2end: config.End2End).output[0];
			...
			using (no_grad())
			{
				...
				foreach (Dictionary<string, Tensor> data in pbar)
				{
					using (NewDisposeScope())
					{
						...
						List<Tensor> nms_results = Ops.non_max_suppression(inference["boxes"], nc: config.NumberClass, conf_thres: 0.1f, iou_thres: 0.7f, end2end: config.End2End).output;
