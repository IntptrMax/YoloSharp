	...
	internal class Segmenter : YoloBaseTaskModel
	{
		...
		internal override List<YoloResult> ImagePredict(Tensor orgImage, float predictThreshold, float iouThreshold)
		{
			...
			if (proto.shape[0] > 0)
			{
				...
				{
					int i = 0;
					Tensor masks = Ops.process_mask(proto[i], nms_results[i][.., 6..], nms_results[i][.., 0..4], new long[] { input.shape[2], input.shape[3] }, upsample: true);
					nms_results[i][.., ..4] = Ops.clip_boxes(nms_results[i][.., ..4], new float[] { orgImage.shape[2], orgImage.shape[3] });
				...
				foreach (Dictionary<string, Tensor> data in pbar)
				{
					using (NewDisposeScope())
					{
						...
						for (int i = 0; i < nms_results.Count; i++)
						{
							...
							long[] size = new long[] { proto.shape[^2], proto.shape[^1] };
