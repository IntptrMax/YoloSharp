...
namespace YoloSharp.Models
{
    internal class Segmenter : YoloBaseTaskModel
    {
        ...
        internal override List<YoloResult> ImagePredict(torch.Tensor orgImage, float predictThreshold, float iouThreshold)
        {
            ...
            List<torch.Tensor> nms_results = Ops.non_max_suppression((torch.Tensor)(inference["boxes"]), nc: this.config.NumberClass, conf_thres: predictThreshold, iou_thres: iouThreshold, end2end: config.End2End).output;
            torch.Tensor proto = inference["proto"];
            ...
            if (proto.shape[0] > 0)
            {
                ...
                {
                    int i = 0;
                    torch.Tensor masks = Ops.process_mask(proto[i], nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(6)], nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 4)], new long[] { input.shape[2], input.shape[3] }, upsample: true);
        ...
        internal override (float[] loss, float[] metrics) Val(YoloDataLoader valDataLoader, AMPWrapper amp, int epoch)
        {
            ...
            using (torch.no_grad())
            {
                ...
                List<torch.Tensor> tpmList = new List<torch.Tensor>();
                ...
                foreach (Dictionary<string, torch.Tensor> data in pbar)
                {
                    using (torch.NewDisposeScope())
                    {
                        ...
                        (List<torch.Tensor> nms_results, _) = Ops.non_max_suppression(preds?.inference["boxes"], nc: config.NumberClass, conf_thres: 0.01f, iou_thres: 0.7f, end2end: config.End2End);
                        ...
                        for (int i = 0; i < nms_results.Count; i++)
                        {
                            ...
                            torch.Tensor coefficient = nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(6)];
                            long[] size = new long[] { proto.shape[proto.shape.Length - 1], proto.shape[proto.shape.Length - 2] };
                            torch.Tensor masks = Ops.process_mask(proto[i], coefficient, pred_bboxes, size);
                            ...
                            torch.Tensor batch_mask = data["masks"];
                            batch_mask = batch_mask[i].squeeze(0);

                            long nl = turn_classes.shape[0];
                            torch.Tensor index = torch.arange(1, nl + 1, device: batch_mask.device).view(nl, 1, 1);
                            ...
                            torch.Tensor miou = Metrics.mask_iou(batch_mask.flatten(1).@float(), masks.flatten(1));
                            torch.Tensor tp_m_epoch = match_predictions(pred_classes, turn_classes, miou);
                ...
                torch.Tensor tp_m_total = torch.cat(tpmList);
                ...
                (torch.Tensor tpm, torch.Tensor fpm, torch.Tensor pm, torch.Tensor rm, torch.Tensor f1m, torch.Tensor apm, torch.Tensor unique_classm, torch.Tensor p_curvem, torch.Tensor r_curvem, torch.Tensor f1_curvem, torch.Tensor xm, torch.Tensor prec_valuesm) = Metrics.ap_per_class(tp_m_total, scores_total, pred_classes_total, true_classes_total);
