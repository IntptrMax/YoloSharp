            ...
            using (no_grad())
            {
                ...
                foreach (Dictionary<string, Tensor> data in pbar)
                {
                    using (NewDisposeScope())
                    {
                        ...
                        Tensor loss_detach = loss.forward(preds?.preds, data).loss_detach;
                        ...
                        List<Tensor> nms_results = Ops.non_max_suppression(preds?.inference["boxes"], nc: config.NumberClass, conf_thres: 0.01f, iou_thres: 0.7f, rotated: true, end2end: config.End2End).output;

                        for (int i = 0; i < nms_results.Count; i++)
                        {
                            Tensor pred_bboxes = torch.cat(new Tensor[] { nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 4)], nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(6, 7)] }, 1);
                            ...
                            Tensor batch_bbox = torch.cat(new Tensor[] { data["bboxes"][batch_idx][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(0, 4)] * scale, data["bboxes"][batch_idx][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(4, 5)] }, 1);
