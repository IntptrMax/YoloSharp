                ...
                foreach (Dictionary<string, Tensor> data in pbar)
                {
                    using (NewDisposeScope())
                    {
                        ...
                        for (int i = 0; i < nms_results.Count; i++)
                        {
                            ...
                            Tensor pred_kpt = nms_results[i][torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(6)].view(new long[] { -1, config.KeyPoint_Num, config.KeyPoint_Dim });
                            ...
                            Tensor area = Ops.xyxy2xywh(batch_bbox)[torch.TensorIndex.Ellipsis, torch.TensorIndex.Slice(2)].prod(1) * 0.53f;
